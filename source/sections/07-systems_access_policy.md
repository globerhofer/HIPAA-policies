...
2. The Human Resources Department, users, and supervisors are required to notify the VP of Engineering to terminate a user's access rights if there is evidence or reason to believe the following (these incidents are also reported on an incident report and is filed with the Privacy Officer):
